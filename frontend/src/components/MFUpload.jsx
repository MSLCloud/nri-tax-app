...
export default function MFUpload({ onNavigate }) {
  ...
  const handleAddFund = () => {
    if (formData.schemeName && formData.units && formData.navPerUnit && formData.costBasis) {
      ...
      setFormData({ schemeName: '', units: '', navPerUnit: '', costBasis: '' });
    }
  };
              ...
              {funds.map((fund, index) => (
                <div key={index} className="flex justify-between items-center p-4 bg-gray-50 rounded">
                  <div>
                    ...
                    <p className="text-sm text-gray-600">{fund.units} units × ${fund.navPerUnit} = ${fund.fmv}</p>
                  </div>
